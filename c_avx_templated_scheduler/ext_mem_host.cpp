 ...
 */
SIMD_TARGET_AVX512
static void accumulate_num_jobs_simd(int            *num_jobs_machine, 
                                      const uint16_t *batch_counts)     
{
    for (int i = 0; i < NUM_MACHINES; i += AVX512_INT32_LANES) {
        ...
        /* Load 16 uint16_t batch counts (32 bytes) and zero-extend to 16 int32_t (64 bytes) */
        __m256i v_u16   = _mm256_maskz_loadu_epi16(valid_mask, batch_counts + i);
        __m512i v_batch = _mm512_cvtepu16_epi32(v_u16);

        /* Load running totals */
        __m512i v_total = _mm512_maskz_loadu_epi32(valid_mask, num_jobs_machine + i);

        /* Accumulate and store */
        __m512i v_sum = _mm512_add_epi32(v_total, v_batch);
        _mm512_mask_storeu_epi32(num_jobs_machine + i, valid_mask, v_sum);
    }
}
/* ═══════════════════════════════════════════════════════════════════════════
 * main()
